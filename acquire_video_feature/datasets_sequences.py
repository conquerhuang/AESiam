...
class Got10kCropped(Dataset):
    ...
    def __init__(self, dataset_path, transforms=None,
        ...
        self.indices = np.arange(0, len(self.seqs))
    ...
    def __getitem__(self, index):
        ...
        if len(val_indices)<2:
            ...
            sequence_dir = self.seqs[index]
            frames = []
            for i in range(len(val_indices)):
                frame = cv2.imread(img_files[val_indices[i]], cv2.IMREAD_COLOR)
                frames.append(frame)
...
class LaSOTCropped(Dataset):          # 需要统计LaSOT的视频序列平均长度和另外两个视频序列的平均长度再做定夺。
    def __init__(self, dataset_path, transforms=None,
        ...
        print('loading metadata from:' + os.path.join(dataset_path, 'lasot_meta.pickl') + '\n')
        with open(os.path.join(dataset_path, 'lasot_meta.pckl'), 'rb') as f:
            lasot_meta = pickle.load(f)
        self.meta_data = lasot_meta['meta_data']
        self.noisy_label = lasot_meta['noisy_label']
        self.target_wh = lasot_meta['target_wh']
...
class MultipleDatasets_sequences(Dataset):
    """
    ...
    def __init__(self, datasets=None, dataset_paths=None, transforms=None, pair_per_seqs=None):
        super(MultipleDatasets_sequences, self).__init__()
        self.transforms = transforms
