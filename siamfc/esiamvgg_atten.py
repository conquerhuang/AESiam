...
from siamfc.backbonesvggnet import ESiamVggAtten, Attention
...
class Net(nn.Module):
    ...
    def __init__(self, backbone, head, atten):
    ...
    def forward(self, z, x):
        z1, z2 = self.backbone(z)  #### class _AlexNet, def forward
        x1, x2 = self.backbone(x)

        # 对特征通道进行感知.
        atten = self.atten(z1)
        z2 = z2 * atten.unsqueeze(dim=2).unsqueeze(dim=2)

        y = self.head(z2, x2)
        return y


class TrackerESiamVggAtten(Tracker):

    def __init__(self, net_path=None, name='Siamvgg', **kwargs):
        super(TrackerESiamVggAtten, self).__init__(name, True)
        ...
        # setup model
        self.net = Net(
            backbone=ESiamVggAtten(),  # out_scale参数，起到对互相关结果加权的作用（互相关的值会很大）
        ...
        for p in self.net.atten.parameters():
    ...
    def init(self, img, box):
        ...
        z1, self.kernel = self.net.backbone(z)
        attention = self.net.atten(z1)
        self.kernel = self.kernel * attention.unsqueeze(dim=2).unsqueeze(dim=2)
