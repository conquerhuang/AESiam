...
def model_transform(model_path=None, save_name=None):
    if model_path is None:
        model_path = r'../pretrained/SiamFC/Siamvgg_oc24.pth'
    if save_name is None:
        save_name = 'transformed_model_siamvgg_oc24.pth'

    tracker = TrackerEvgg_oc(net_path=None, name='ESiamvgg')
    model_dict = tracker.net.state_dict()

    saved_model = torch.load(model_path)
    state_dict = {}

    # transform state dict in share features
    for i, (kv1, kv2) in enumerate(zip(saved_model.items(), model_dict.items())):
        if i <= 62:
            k1, v1 = kv1[0], kv1[1]
            k2, v2 = kv2[0], kv2[1]
            print(k1, '\t', k2)
    ...
    conv_weight = saved_model['backbone.features.30.weight']
    conv_bias = saved_model['backbone.features.30.bias']
    squeeze_weight = saved_model['backbone.features.31.weight']
    squeeze_bias = saved_model['backbone.features.31.bias']
    conv10_weight, conv10_bias = kernel_simplify(conv_weight, conv_bias, squeeze_weight, squeeze_bias, groups=1)

    model_dict['backbone.cluster_feature.0.weight'] = conv10_weight
    model_dict['backbone.cluster_feature.0.bias'] = conv10_bias

    model_dict['head.bn_adjust.weight'] = saved_model['head.bn_adjust.weight']
    model_dict['head.bn_adjust.bias'] = saved_model['head.bn_adjust.bias']
    model_dict['head.bn_adjust.running_mean'] = saved_model['head.bn_adjust.running_mean']
    model_dict['head.bn_adjust.running_var'] = saved_model['head.bn_adjust.running_var']
    model_dict['head.bn_adjust.num_batches_tracked'] = saved_model['head.bn_adjust.num_batches_tracked']
...
def model_transform1(model_path=None, squeeze_rate=None, save_name=None):
    if model_path is None:
    ...
    tracker = TrackerEvgg_oc(net_path=None, name='FDSiamFC_8l_lite', squeeze_rate=squeeze_rate)
    model_dict = tracker.net.state_dict()
...
if __name__ == '__main__':
    ...
    model_path = r'../pretrained/SiamFC/Siamvgg_oc24.pth'
    model_transform(model_path=model_path)
