...
from siamfc.backbonesvggnet import EVGGNet_oc
...
class Net(nn.Module):
    ...
    def forward(self, z, x):
        z = self.backbone(z)  #### class _AlexNet, def forward
        x = self.backbone(x)
        return self.head(z, x)


class TrackerSiamvgg_cluster(Tracker):

    def __init__(self, net_path=None, name='Siamvgg', **kwargs):
        super(TrackerSiamvgg_cluster, self).__init__(name, True)
        ...
        # setup model
        self.net = Net(
            backbone=EVGGNet_oc(),  # out_scale参数，起到对互相关结果加权的作用（互相关的值会很大）
        ...
        for p in self.net.backbone.cluster_feature.parameters():
    ...
    def train_step(self, batch, backward=True):
        ...
        # froze running mean and running var
